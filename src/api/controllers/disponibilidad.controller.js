...
export const deleteDisponibilidadEspacio_vc_bb = async (req_vc_bb, res_vc_bb) => {
  try {
    const { id } = req_vc_bb.params;
    const result_vc_bb = await db_vc_bb.run_vc_bb(`DELETE FROM td_DisponibilidadEspacio_bb_vc WHERE ID_DisponibilidadEspacio_bb_vc = ?;`, [id]);
    if (result_vc_bb.changes === 0) return res_vc_bb.status(404).json({ message: "Disponibilidad espacio no encontrada" });
